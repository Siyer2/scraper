...
function getGeneralEducation(db, { faculty, year, studyLevel }) {
    return new Promise(async (resolve, reject) => {
        try {
            const programInfo = { faculty, year, studyLevel };
            var config = {
                method: 'post',
                url: `${getEndpoint()}/generalEducation`,
                headers: {
                    'Content-Type': 'application/json'
                },
                data: { programInfo }
            ...
            axios(config)
                ...
                .catch(async function (error) {
                    console.log("AXIOS ERROR GETTING GENERAL EDUCATION", error.response.status);
                    // If failed 
                    await getGeneralEducation(db, { faculty, year, studyLevel });
                    reject(error.response.status);
                });
